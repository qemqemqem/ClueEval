...
def get_random_details() -> str:
    ...
    random_persons = random.sample(random_person_elements, 5)

    killer, victim = random_persons[0], random_persons[1]

    story = f"This is a mystery story in the style of a golden age classic, and it features the following elements:"
    for element in random_crimes + random_places + random_persons:
        story += f"\n- {element}"

    story += f"\n\nThe central story is that a crime was commited with a {random.choice(random_crimes)} in the {random.choice(random_places)} by {killer}, killing {victim}. But there's shenanigans going on with the other stuff, too."

    return story


def main():
    diversity_prompt = get_random_details()
